...
export default (toLog, options = {}) => {
  if (toLog.message.startsWith('START RequestId: ')) {
    ...
    if (!toLog.requestId && toLog.record && toLog.record.requestId) {
      toLog.requestId = toLog.record.requestId
    }
    if (typeof toLog.message !== 'string') {
      if (toLog.type === 'platform.start') {
        toLog.message = `START RequestId: ${toLog.requestId}`
      } else if (toLog.type === 'platform.initStart' && toLog.record) {
        toLog.message = `INIT_START Runtime Version: ${toLog.record.runtimeVersion}`
      } else if (toLog.type === 'platform.end') {
        toLog.message = `END RequestId: ${toLog.requestId}`
      } else if (toLog.type === 'platform.report') {
        toLog.message = `REPORT RequestId: ${toLog.requestId}`
        if (toLog.record && toLog.record && toLog.record.metrics) {
          toLog.duration = toLog.record.metrics.durationMs
          toLog.billedDuration = toLog.record.metrics.billedDurationMs
          toLog.memorySize = toLog.record.metrics.memorySizeMB
          toLog.maxMemoryUsed = toLog.record.metrics.maxMemoryUsedMB
          toLog.initDuration = toLog.record.metrics.initDurationMs
          toLog.memoryDifference = toLog.memorySize - toLog.maxMemoryUsed
        }
      }
    }
