...
export default (toLog, options = {}) => {
  if (toLog.message.startsWith('START RequestId: ')) {
    ...
    if (toLog.message && toLog.message.startsWith('{') && toLog.message.endsWith('}')) {
      const parsedMessageJSON = JSON.parse(toLog.message)
      for (const member in parsedMessageJSON) toLog[member] = parsedMessageJSON[member]
    }
